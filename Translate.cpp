		...
		if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
		{
			...
			for (i = 0; i < 2; i++)
			{
					...
					localPosition.y <= buttons[i]->getPos().y + 49)
				{
					...
					{
						String str = L"\tЗапомните слова и перевод";
...
void Translate::windows()
{
	...
	imageFon.loadFromFile("..\\prog\\Big_Ben.jpg");
...
void Translate::windowsTranslate()
{
	window.clear();
	Image image, imageFon;
	imageFon.loadFromFile("..\\prog\\Monuments.jpg");
	Texture texture, textureFon;
	textureFon.loadFromImage(imageFon);
	window.setFramerateLimit(60);
	Sprite splite;
...
void Translate::windowsTraining()
{
	window.clear();
	Image image, imageFon;
	imageFon.loadFromFile("..\\prog\\London-Clock.jpg");
	Texture texture, textureFon;
	textureFon.loadFromImage(imageFon);
	window.setFramerateLimit(60);
	Sprite splite;
